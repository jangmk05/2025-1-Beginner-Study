...
- 답: 1. 기본적인 병합 방식, 두 브랜치의 변경 사항을 하나의 새로운 커밋으로 병합 2. 여러개의 커밋을 하나의 커밋으로 압축(squash)하여 병합 3. 원본 브랜치 위에 변경 사항을 재정렬한 후, 병합하는 방식.
...
- 답: 현재 브랜치는 develop이다. HEAD가 develop 브랜치를 가리키고 있다. 최근 커밋의 해시값은 a1s2d3f이다. 최근 커밋 메시지는 docs: readme 추가 이다.
...
- 답: 협업에서는 커밋 기록을 보존하는 것이 중요하므로 다른 사람 작업에 영향을 주지 않는 revert를 사용
